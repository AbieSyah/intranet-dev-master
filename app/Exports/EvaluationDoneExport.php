...
class EvaluationDoneExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles
{
    protected $tahun;
    ...
    {
        $query = Evaluation::with(['employee.department', 'employee.section', 'employee.position', 'employee.building'])
                    ->where('evaluations.status', 'DONE');
                    
        if (!empty($this->tahun)) {
            $query->whereYear('eval_end', $this->tahun); 
        }
