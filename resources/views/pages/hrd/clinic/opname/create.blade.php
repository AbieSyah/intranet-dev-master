...
<div class="row">
  <div class="col-12">
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
      <h4 class="mb-sm-0">Medicine Transaction Opname</h4>
...
<div class="row">
  <div class="col-lg-12">
    <div class="card">
      <div class="card-body">
        <form id="form-opname" method="POST" action="{{route('clinic.opname.store')}}">
            @csrf
            @method('PUT')
                        ...
                        <tbody id="newlink">
                            <tr id="1" class="product">
                                ...
                                <td>
                                    <div class="mb-2">
                                        <div class="input-group">
                                            <input type="number" class="form-control text-sm" id="jml_stock"
                                                name="jml_stock" value="0" style="Background-color: #eff2f7;" readonly>
...
@section('javascript')
    <script>
        ...
        $( "#btn-submit" ).click(function() {
            $("#form-opname").submit(function () {
                $('#staticBackdrop').modal('show', true);
            });
        });
    </script>
    <script>
        $('#id_drug-dropdown').on('change', function() {
            var id_drug = this.value;
            $.ajax({
                url: "{{ route('clinic.opname.select.stock') }}",
                type: "POST",
                data: {
                    id_drug: id_drug,
                    _token: '{{ csrf_token() }}'
    ...
    <script>
        ...
        function new_link() {
            ...
            $(document).ready(function() {
                $('#id_drug-dropdown-'+ count + '').on('change', function() {
                    var id_drug = this.value;
                    $.ajax({
                        url: "{{ route('clinic.opname.select.stock') }}",
                        type: "POST",
                ...
                t = (e.id = count, e.className = "product", '<tr>'+
                    '<th scope="row" class="product-id">' + count +'</th>'+
                        ...
                        '<td>'+
                            '<div class="mb-2">'+
                                '<div class="input-group">'+
                                    '<input type="number" class="form-control text-sm" id="jml_stock-'+ count +'" name="jml_stock" value="0" style="Background-color: #eff2f7;" readonly>'+
                                    '<span class="input-group-text" id="basic-addon2"><i class="las la-capsules"></i></span>'+
                                '</div>'+
                            '</div>'+
                        '</td>'+
